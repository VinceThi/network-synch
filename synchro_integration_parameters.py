...
# Time parameters
numberoftimepoints = 20000  # 5000
timelist = np.linspace(0, 30, numberoftimepoints)       # attention, en diminuant alpha, ça stabilise plus lentement
deltat = timelist[1] - timelist[0]


# Structural parameter of the SBM
N = 200   #256  # 1000
m = 100    #128  # 500  # int(N/2)  # Donc beta = 1/2
sizes = [m, N-m]
n1 = sizes[0]
n2 = sizes[1]
...
rho_array = np.linspace(0, 1, 20)          # Average density. See Jean-Gab article on finite size analysis of the detectability limit of the SBM
delta_array = np.linspace(0, 0.7, 20)      # p - q. See Jean-Gab article on finite size analysis of the detectability limit of the SBM
