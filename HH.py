...
def HHequations(w, t, N, adjacencymatrix, kvec, gsyn, Isynvec, Cm=1):
    """
...
def integrate_HH_CrankN(w, t, N, adjacencymatrix, kvec, gsyn, Isynvec, nonlinsystem_method="fsolve", Cm=1):#w0, time, g_ion, typeofcurrent, z, nonlinsystem_method):
    """
    ...
    def ionic_currents(V, m, h, n):
        return (Isynvec - INa(V, m, h) - IK(V, n) - IL(V) + gsyn*(np.dot(adjacencymatrix, V) - kvec * V)) / Cm
...
def MPS_by_block_without_timeavg(phase_matrix, sizes):
    """
    Computes the Multivariate Phase Synchronization (MPS) as defined by Jalili et Al. in
    Synchronization of EEG: Bivariate and Multivariate Measures
    :param phase_matrix: Matrix computed with the function '' phase_from_hilbert '' defined above.
...
def find_HH_synchro_transition(w0, timelist, N, sizes, adjacencymatrix, kvec, gsyn_array, Isynvec):
    """Find synchronisation transition MPS vs gsyn (kind of synaptique conductance (coupling)) for the Hodgkin-Huxley model."""
    ttrans = timer.clock()
    MPS_vs_gsyn = np.zeros(len(gsyn_array))
    MPS_blocks_vs_gsyn = np.zeros((len(gsyn_array), len(sizes)))

    i = 0
    for g in gsyn_array:
        t0 = timer.clock()

        solution = odeint(HHequations, w0, timelist, args=(N, adjacencymatrix, kvec, g, Isynvec,))
        V = solution[:, 0:N]
        print("Time to integrate HH for g = {}: {} seconds".format(g, timer.clock()-t0))

        phase_mat = phase_from_hilbert(V, N)

        MPS_value = MPS(phase_mat)
        MPS_blocks_values = MPS_by_block(phase_mat, sizes)
        print('MPS : ', MPS_value)
        print('MPS blocks : ', MPS_blocks_values)

        MPS_vs_gsyn[i] = MPS_value

        MPS_blocks_vs_gsyn[i, :] = MPS_blocks_values

        #print(MPS_blocks_vs_gsyn)
        
        i += 1

    return MPS_vs_gsyn, MPS_blocks_vs_gsyn, (timer.clock()-ttrans)/60


############################### Plot functions #########################################################################
def figureHH_with_gating_variables_and_currents(w0, timelist, N, sizes, adjacencymatrix, kvec, gsyn, Isynvec):

    solution = odeint(HHequations, w0, timelist, args=(N, adjacencymatrix, kvec, gsyn, Isynvec,))
...
def figureHH(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec):
    # Odeint solutions
    t0 = timer.clock()
    solution = odeint(HHequations, w0, timelist, args=(N, adjacencymatrix, kvec, gsyn, Isynvec,))
    ...
    h = solution[:, 2*N : 3*N]
    n = solution[:, 3*N : 4*N]
    print("Time to integrate HH:{} seconds".format(timer.clock()-t0))
    phase_mat = phase_from_hilbert(V, N)
    print('MPS : ', MPS(phase_mat))

    # Not finished
    ## Crank-Nicolson solutions
    #solution = integrate_HH_CrankN(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec)
    #V = solution[0]
    #m = solution[1]
    #h = solution[2]
    #n = solution[3]
...
def synchro_transition_HH(w0, timelist, N, sizes, adjacencymatrix, kvec, gsyn_array, Isynvec):
    plt.figure(figsize=(8, 5))
    plt.rc('font', family='serif')

    MPS_transitions = find_HH_synchro_transition(w0, timelist, N, sizes, adjacencymatrix, kvec, gsyn_array, Isynvec)
    ...
    if messagebox.askyesno("Python", "Would you like to save the parameters, the data and the plot?"):
        ...
        fig.savefig("synchro_data/HH_data/{}_{}_figure.jpg".format(timestr, file))

        f = open('synchro_data/HH_data/{}_{}_parameters.txt'.format(timestr, file), 'w')
        f.writelines(tosavelist)
        f.close()

        with open('synchro_data/HH_data/{}_{}_MPS_transitions_global.json'.format(timestr, file), 'w') as outfile:
            json.dump(MPS_transitions[0].tolist(), outfile)
...
if __name__ == '__main__':
    ...
    figureHH(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec)
    """


    #################################### Plot synchro transition #######################################################
    # Time parameters
    numberoftimepoints = 10000
    timelist = np.linspace(0, 200, numberoftimepoints)
    deltat = timelist[1] - timelist[0]
    ...
    synchro_transition_HH(w0, timelist, N, sizes, adjacencymatrix, kvec, gsyn_array, Isynvec)
    ...
    #print(w0)
    #figureHH_with_gating_variables_and_currents(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec)
...
#
################################ Plot functions #########################################################################
#def figureHH_with_gating_variables_and_currents(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec):
#
#    solution = odeint(HHequations, w0, timelist, args=(N, adjacencymatrix, kvec, gsyn, Isynvec,))
#
#    V = solution[:, 0 : N]
...
#    h = solution[:, 2*N : 3*N]
#    n = solution[:, 3*N : 4*N]
#    print('Time taken : ', timer.clock()- starttime)
#    # Not finished
#    ## Crank-Nicolson solutions
#    #solution = integrate_HH_CrankN(w0, timelist, N, adjacencymatrix, kvec, gsyn, Isynvec)
#    #V = solution[0]
